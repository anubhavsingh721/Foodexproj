...
app.get("/", function (req, res) {
  if (!loggedIn) res.render("home", { check: "nope" });
  else res.render("home", { check: "yes" });
});
...
app.get("/mycart", function (req, res) {
  ...
  con.query("SELECT * FROM recipe", function (err, results, fields) {
    ...
    if (!loggedIn) {
      res.render("mycart", { check: "nope", cards: cardDetails, status: status, currentOrderId: currentOrderId  });
    } else {
      res.render("mycart", { check: "yes", cards: cardDetails, status: status, currentOrderId: currentOrderId });
    }
  });
});
...
app.post("/mycart", function (req, res) {
  ...
  if (flag === false) {
    cart.push({ index: newIndex, quantity: 1 });
  }
});
...
app.get("/updateProfile", function (req, res) {
  res.render("updateProfile", { profile: currentUser });
});
...
app.get("/admin/orders/:status", function (req, res) {
  ...
  con.query(sql, function(err, results) {
    ...
    res.render("adminorders", {orderArray: orderArray});
  }) 
})
...
app.get("/admin/order/:orderid", function (req, res) {
  ...
  async function makeQuery() {
    ...
    res.render("adminspecificorder", {userDetails: userDetails, orderDetails: orderDetails});
  }
  makeQuery();
}) 
...
app.get("/admin/showfoodmenu", function (req, res) {
  ...
  async function makeQuery() {  
    ...
    for(var i = 0; i < results.length; i++) {
      ...
      var newObject = { categoryName: tempCatName, recipeName: results[i].Name};
      menuArray.push(newObject);
    }
    res.render("adminshowfoodmenu", {menuArray: menuArray});
  }
  makeQuery();  
});
...
app.get("/admin/addrecipe", function(req, res) {
  ...
  con.query("SELECT Name FROM category", function(err, results) {
    ...
    res.render("adminaddrecipe", {categoryArray: categoryArray});
  })
})
...
app.get("/admin/showfoodcategory", function (req, res) {
  ...
  con.query("SELECT Name FROM category", function(err, results) {
    if(err) throw err;
    results.forEach((result) => {
      var newObject = { Name: result.Name };
      categoryArray.push(newObject);
    })
    res.render("adminshowfoodcategory", {categoryArray: categoryArray});
  })
});
...
app.get("/admin/users", function (req, res) {
  ...
  con.query("SELECT FirstName, LastName, PhoneNum, Email FROM customer", function(err, results) {
    ...
    res.render("adminUsers", { usersArray: usersArray });
  })
});
